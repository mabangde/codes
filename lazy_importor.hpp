...
#define LI_MODULE(name) ::li::detail::lazy_module<::li::detail::khash2(name)>()
...
#define LAZY_IMPORTER_TOLOWER_IMPL(c) (c >= 'A' && c <= 'Z' ? (c | (1 << 5)) : c)
...
namespace li { namespace detail {
    ...
    struct hash_t {
        ...
        constexpr static value_type         prime   = /*16777619*/25683967;
                                                                     ...
                                                                     bool caseInsensitive = false) noexcept
        {
            return static_cast<hash_t::value_type>(
                (value ^ (caseInsensitive? LAZY_IMPORTER_TOLOWER_IMPL(c) : c)) *
	...
	template<class CharT = char>
	LAZY_IMPORTER_FORCEINLINE constexpr hash_t::value_type
		khash2(const CharT* str, hash_t::value_type value = hash_t::offset) noexcept
	{
		return (*str ? khash(str + 1, hash_t::single(value, *str, true)) : value);
	}
    ...
    LAZY_IMPORTER_FORCEINLINE hash_t::value_type hash(
        const win::UNICODE_STRING_T& str, bool caseInsensitive) noexcept
    {
        auto       first = str.Buffer;
        const auto last  = first + (str.Length / sizeof(wchar_t));
        auto       value = hash_t::offset;
        for(; first != last; ++first)
            value = hash_t::single(value, static_cast<char>(*first), caseInsensitive);
        ...
        const char* str) noexcept
    {
        ...
        for(; *str != '.'; ++str)
            module_and_function.first = hash_t::single(module_and_function.first, *str, true);

        ++str;

        for(; *str; ++str)
            module_and_function.second = hash_t::single(module_and_function.second, *str, false);
...
#error Unsupported platform. Open an issue and I'll probably add support.
    ...
    struct safe_module_enumerator {
        ...
        safe_module_enumerator(const detail::win::LDR_DATA_TABLE_ENTRY_T* ldr) noexcept
            : value(ldr->load_order_next()), head(/*value*/ldr->load_order_next())
        {}

        LAZY_IMPORTER_FORCEINLINE void reset() noexcept
        {
            value = /*head->load_order_next()*/head;
        }
    ...
    struct lazy_module : lazy_base<lazy_module<Hash>> {
        ...
        LAZY_IMPORTER_FORCEINLINE static T get() noexcept
        {
            Enum e;
            do {
                if(hash(e.value->BaseDllName, true) == Hash)
    ...
    struct lazy_function : lazy_base<lazy_function<Hash, T>, T> {
        ...
        static inline bool is_api_ms_win(const char* addr)
        {
            constexpr auto apimswinHash = khash2("api-ms-win-");
            uint32_t thisHash = hash_t::offset;
			for (auto idx = 0; idx < sizeof("api-ms-win-") - 1 && addr[idx]; ++idx)
				thisHash = hash_t::single(thisHash, addr[idx], true);
            return thisHash == apimswinHash;
        }
        ...
        LAZY_IMPORTER_FORCEINLINE static F forwarded() noexcept
        {
            ...
            do {
                ...
                if(!module_hash || hash(name, true) == module_hash) {
                    const exports_directory exports(e.value->DllBase);

                    if(exports) {
                        bool reset_continue = false;
                        auto export_index = exports.size();
                        while(export_index--)
                            if(hash(exports.name(export_index)) == function_hash) {
                                ...
                                if(exports.is_forwarded(addr)) {
                                    if (/*_strnicmp(addr, "api-ms-win", 10) == 0*/
                                        is_api_ms_win(addr))
                                    {
                                        break;
                                    }
