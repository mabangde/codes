...
def write_csv(csv_data):
    list_of_headers = ['url', 'domain', 'gitlink']
    if os.path.isfile(save_filename):
        with open(save_filename, 'a', newline='', encoding='utf-8-sig') as my_file:
            w = csv.DictWriter(my_file, fieldnames= list_of_headers)
            for z in csv_data:
                w.writerow(z)
    else:
        with open(save_filename, 'w', newline='', encoding='utf-8-sig') as my_file:
            w = csv.DictWriter(my_file, fieldnames= list_of_headers)
            w.writeheader()
            for z in csv_data:
                w.writerow(z)
